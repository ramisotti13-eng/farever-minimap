...
// #109: told by the hl_pump worker which character is being played, using the
// same debounced profile key the mod's own per-character files use. Emits
// on_event("character_login", {key, name}) on an actual change, and backs
// farever.player.character_key() / farever.store.get_char|set_char.
// A plugin loaded (or hot-reloaded) after the character settled will not see
// the event - it should read farever.player.character_key() in on_init.
void plugins_set_character(const char* key, const char* name);
// #87: target_name = victim name (== farever.target.name()); blocked = the
// blocked portion of the hit (DamageResult._block). Both may be empty/0.
...
// #70: per-heal event (on_event("heal_dealt", {skill, amount, is_crit,
// target})). #87 added target = the heal recipient name.
void plugins_emit_heal_dealt(const char* skill_name, double amount,
                             bool is_crit, const char* target_name);
// #70: shield-applied event (on_event("shield_applied", {skill, amount})),
// synthesized by hero_state when the player's total active-status absorb rises
// from ~0 to positive. `skill` is the granting status kind.
