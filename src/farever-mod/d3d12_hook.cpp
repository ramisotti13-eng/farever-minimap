// Kiero-style D3D12 vtable hook. Discover Present / ResizeBuffers /
// ExecuteCommandLists from a throwaway swap chain, then MinHook them.
//
// The Present detour always drives the HashLink tick (so heap reads
// happen on a thread the GC knows about - see
// feedback_hashlink_pump_thread.md) when the hl_pump worker is inactive.
//
// The set of hooks installed depends on the render backend:
//   Dcomp         - Present only (tick driver). The overlay renders on
//                   its own thread into its own composition swapchain
//                   (overlay_window.cpp); we never touch the game's.
//   GameSwapchain - Present (tick + overlay submit into the game's back
//                   buffer) + ResizeBuffers (recreate RTVs on resize) +
//                   ExecuteCommandLists (capture the game's DIRECT queue
//                   so the overlay submits in Present order). This is the
//                   v0.4.x path: no compositor blend, Proton-friendly,
//                   dodges AMD MPO. The original DX12Driver.present AV
//                   that drove the move to Dcomp was the HashLink-GC race
//                   during STATE READS (fixed by hl_pump), not the render.
...
HRESULT STDMETHODCALLTYPE hook_present(IDXGISwapChain3* self,
    ...
    // When the background worker is active (default) it drives the ticks
    // on its own HL-registered thread; skip the Present-side reads to
    // avoid double-ticking. Same in both backends.
    if (!hl_pump_is_active()) {
        damage_tick();
        hero_state_tick();
        target_state_tick();
        heal_tick();         // issue #57 - local-player heal source
    }
    // GameSwapchain only: render + submit the overlay into the game's
    // own back buffer, on the game's captured DIRECT queue. We defer the
    // first frame until ExecuteCommandLists has captured that queue, so
    // the overlay never inits against a freshly-created own queue (which
    // would break Present ordering on the game swapchain). DCOMP renders
    // on its own thread and never goes through here.
    if (overlay_backend() == RenderBackend::GameSwapchain) {
        ID3D12CommandQueue* q = g_captured_queue.load();
        if (q) overlay_on_present(self, q);
    }
...
bool d3d12_hook_install() {
    ...
    logf("d3d12: hooks installed (backend=%s, Present=%p%s)",
         game_swapchain ? "game-swapchain" : "dcomp", present_ptr,
         game_swapchain ? " +ResizeBuffers +ExecuteCommandLists" : " tick-only");
    return true;
}
