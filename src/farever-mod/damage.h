...
struct DamageEvent {
    ...
    char           target_name[64];  // #87: victim name, same value
                                     // farever.target.name() gives (kind, or
                                     // boss class name); empty if unresolved
    double         blocked;       // #87: DamageResult._block @96 - the blocked
                                  // portion of this hit (0 when nothing blocked)
};
