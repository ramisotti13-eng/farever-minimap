...
//     knows about). The hook does no rendering, no submission,
//     and no queue capture.
...
void render_thread_main() {
    ...
    // v0.6.1 (#212): debounce swap-chain resizes. An in-game resolution
    // change fires a WM_SIZE storm - GetClientRect flutters through many
    // intermediate sizes within a few frames. The old code resized on
    // every differing size, which spammed ResizeBuffers and cascaded
    // into DXGI_ERROR_DEVICE_REMOVED on our swap chain, then a game-side
    // allocTexture crash. Now we only resize once the observed size has
    // held steady for a short window, and never more often than the min
    // gap. If our device is lost anyway, we stop resizing + presenting
    // and detach the visual so we don't keep feeding the cascade.
    UINT pending_w = current_w, pending_h = current_h;
    auto pending_since  = std::chrono::steady_clock::now();
    auto last_resize_at = std::chrono::steady_clock::time_point{};
    constexpr auto kResizeDebounce = std::chrono::milliseconds(250);
    constexpr auto kResizeMinGap   = std::chrono::milliseconds(200);
    ...
    while (!g_win.stop.load(std::memory_order_acquire)) {
        // #212: set when the game window's client size no longer matches
        // our swap chain (a resolution change is in flight). While true we
        // skip presenting so we don't push mismatched-size frames through
        // DCOMP onto a HWND the game is mid-resizing - that keeps only the
        // game's swap chain active during the volatile transition window.
        ...
        // debounced so a WM_SIZE storm can't spam ResizeBuffers (#212).
        RECT rc;
        if (!device_lost && GetClientRect(g_win.game_hwnd, &rc)) {
            ...
            if (new_w > 0 && new_h > 0) {
                ...
                if (new_w != pending_w || new_h != pending_h) {
                    // Size moved since we last looked - (re)start the
                    // stability timer and track the latest target.
                    pending_w = new_w;
                    pending_h = new_h;
                    pending_since = rnow;
                } else if ((new_w != current_w || new_h != current_h) &&
                           (rnow - pending_since) >= kResizeDebounce &&
                           (rnow - last_resize_at) >= kResizeMinGap) {
                    // Target held steady long enough - commit one resize.
                    logf("overlay_window: game window settled %ux%u -> %ux%u, "
                    ...
                    if (hr == DXGI_ERROR_DEVICE_REMOVED ||
                        ...
                        HRESULT reason = g_win.device
                            ? g_win.device->GetDeviceRemovedReason() : hr;
                        logf("overlay_window: ResizeBuffers device lost "
                             "hr=0x%08lx reason=0x%08lx - halting overlay "
                             "render to avoid crash cascade",
                        ...
                        logf("overlay_window: ResizeBuffers failed hr=0x%08lx "
                             "(will retry after gap)", (unsigned long)hr);
                    } else {
        ...
        if (g_win.visible.load() && hero_locked_now && !s_fg_detached &&
            ...
            if (FAILED(hr)) {
                hr = g_win.swap_chain->Present(1, 0);
            }
            if (hr == DXGI_ERROR_DEVICE_REMOVED ||
                hr == DXGI_ERROR_DEVICE_RESET) {
                HRESULT reason = g_win.device
                    ? g_win.device->GetDeviceRemovedReason() : hr;
                logf("overlay_window: Present device lost hr=0x%08lx "
                     "reason=0x%08lx - halting overlay render",
                     (unsigned long)hr, (unsigned long)reason);
                device_lost = true;
                if (g_win.dcomp_visual && g_win.dcomp_device) {
                    g_win.dcomp_visual->SetContent(nullptr);
